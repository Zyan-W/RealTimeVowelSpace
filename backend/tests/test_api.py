...
def test_corpus_endpoint_shape():
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["id"] == "american-english"
    assert body["language"] == "American English"
    assert body["tokens"][0]["word"]
    assert body["tokens"][0]["references"]["american"]["f1"]


def test_corpora_endpoint_returns_english_systems_and_japanese():
    client = TestClient(app)
    response = client.get("/api/corpora")
    assert response.status_code == 200
    corpus_ids = {item["id"] for item in response.json()}
    assert {"american-english", "british-english", "japanese"} <= corpus_ids
